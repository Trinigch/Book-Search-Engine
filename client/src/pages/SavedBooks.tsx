...
import { REMOVE_BOOK } from '../apollo/mutations';
...
const SavedBooks = () => {
  ...
  // Set up mutation to remove books
  const [removeBook] = useMutation(REMOVE_BOOK);

  // Create function to handle deleting books
  const handleDeleteBook = async (bookId: string) => {
    try {
      const token = Auth.loggedIn() ? Auth.getToken() : null;
      ...
      await removeBook({
        variables: { bookId },
        update: (cache) => {
          // Update Apollo cache to remove the deleted book
